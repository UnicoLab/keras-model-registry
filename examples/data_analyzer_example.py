...
def create_example_dataset():
    ...
    data["correlated_2"] = data["correlated_1"] * 0.8 + np.random.normal(0, 0.2, n_samples)
    
    # Categorical features with different cardinality
    data["category_low"] = np.random.choice(["A", "B", "C"], n_samples)
    data["category_medium"] = np.random.choice([f"Val_{i}" for i in range(20)], n_samples)
    data["category_high"] = np.random.choice([f"ID_{i}" for i in range(500)], n_samples)
    
    # Date features
    base_date = pd.Timestamp("2020-01-01")
    data["date"] = [base_date + pd.Timedelta(days=i) for i in np.random.randint(0, 365*2, n_samples)]
    ...
    missing_indices = np.random.choice(n_samples, size=int(n_samples * 0.3), replace=False)
...
def main():
    ...
    print(f"Example dataset created at: {data_path}")
    ...
    for characteristic, values in stats["characteristics"].items():
        if characteristic == "feature_interaction":
            print(f"{characteristic}: {len(values)} correlated feature pairs")
        elif values and values != ["all"]:
            print(f"{characteristic}: {len(values)} features")
    
    print("\n=== Layer Recommendations ===")
    for characteristic, layers in result["recommendations"].items():
        print(f"\nFor {characteristic}:")
        for layer, description, use_case in layers:
    ...
    print("\n=== Adding Custom Layer Recommendation ===")
    ...
    print("Updated recommendations for high_cardinality_categorical:")
    for layer, description, use_case in updated_recommendations["high_cardinality_categorical"]:
        print(f"  - {layer}: {description}")
