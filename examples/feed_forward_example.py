...
def create_preprocessing_model(input_dim: int) -> Model:
    ...
    x = layers.Dense(32, activation='relu', name='preprocessing_dense_1')(preprocessing_input)
...
def run_feed_forward_example():
    ...
    try:
        ...
        X_train = {name: train_df[name].values.astype(np.float32) for name in feature_names}
        ...
        X_test = {name: test_df[name].values.astype(np.float32) for name in feature_names}
