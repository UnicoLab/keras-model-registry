...
def create_kdp_preprocessor(csv_path: Path) -> TabularDataProcessor:
    ...
    processor = TabularDataProcessor(
        ...
        categorical_columns=['education', 'employment_status', 'city_tier', 'has_loan', 'owns_property'],
...
def test_raw_data_prediction(
    ...
    for i, test_case in enumerate(raw_test_cases, 1):
        ...
        print(f"   💡 Recommendation: {'APPROVE' if loan_probability > 0.5 else 'REJECT'}")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        feature_names = [col for col in df.columns if col != 'loan_approval_probability']
        ...
        print(f"📊 Final test metrics: MSE={metrics['mse']:.4f}, MAE={metrics['mae']:.4f}")
