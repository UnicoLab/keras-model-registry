...
class NeuralObliviousDecisionEnsemble(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        logger.debug(
            "NeuralObliviousDecisionEnsemble built: {} trees, depth {}, {} leaves per tree.",
            self.num_trees,
            self.tree_depth,
            self.num_leaves,
        )
    ...
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        decision_logits = tf.tensordot(
            inputs,
            self.decision_weights,
            axes=[[1], [2]],
        )  # (batch, num_trees, tree_depth)
        decision_logits = (
            decision_logits - self.decision_biases
        )  # (batch, num_trees, tree_depth)
        ...
        mask = tf.cast(
            tf.bitwise.right_shift(bin_format, tf.range(self.tree_depth)) & 1,
            tf.float32,
        ...
        leaf_probs = tf.math.pow(decisions_expanded, mask_expanded) * tf.math.pow(
            1 - decisions_expanded,
            1 - mask_expanded,
        )
