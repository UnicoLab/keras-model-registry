...
class RankingModel(Model):
    def __init__(self, feature_dim):
        ...
        self.output_layer = layers.Dense(
            units=1,
            activation="linear",
        )  # Linear activation to allow negative values
...
class UnifiedRecommendationModelWithPreprocessing(Model):
    def __init__(
        ...
        item_dim,
        feature_dim,
        preprocessing_model_user,
        preprocessing_model_item,
    ):
        super().__init__()
        self.num_users = num_users
        self.num_items = num_items
        ...
        self.two_tower_weight = tf.Variable(
            1.0,
            trainable=True,
            name="two_tower_weight",
        )
        ...
        for feature_name, shape in input_shapes.items():
            if feature_name in CONF.data.FEATURES_SPECS_USER:
                ...
                if len(shape) != 2 or shape[1] != 1:
                    raise ValueError(
                        f"Expected shape (None, 1) for feature {feature_name}, got {shape}",
                    )
    ...
    def call(self, inputs):
            ...
            try:
                ...
                if hasattr(v, "numpy"):
                    ...
                    if v.dtype in (tf.float32, tf.float64, tf.int32, tf.int64):
                        print(
                            f"    Min/Max: {tf.reduce_min(v).numpy()}/{tf.reduce_max(v).numpy()}",
                        )
            except Exception:
                pass
        ...
        try:
            if hasattr(user_features, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(user_features).numpy()}/{tf.reduce_max(user_features).numpy()}",
                )
        except Exception:
            pass
        ...
        try:
            if hasattr(item_features, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(item_features).numpy()}/{tf.reduce_max(item_features).numpy()}",
                )
        except Exception:
            pass

        # Shape enforcement
        user_features = tf.ensure_shape(
            user_features,
            (None,) + self.user_feature_shape,
        )
        item_features = tf.ensure_shape(
            item_features,
            (None,) + self.item_feature_shape,
        )
        ...
        ranking_input = tf.ensure_shape(
            ranking_input,
            [None, self.user_feature_shape[0] + self.item_feature_shape[0] + 1],
        )
        ...
        try:
            if hasattr(collab_scores, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(collab_scores).numpy()}/{tf.reduce_max(collab_scores).numpy()}",
                )
        except Exception:
            pass
        ...
        try:
            if hasattr(retrieval_scores, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(retrieval_scores).numpy()}/{tf.reduce_max(retrieval_scores).numpy()}",
                )
        except Exception:
            pass
        ...
        try:
            if hasattr(ranking_input, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(ranking_input).numpy()}/{tf.reduce_max(ranking_input).numpy()}",
                )
        except Exception:
            pass
        ...
        try:
            if hasattr(quantity_prediction, "numpy"):
                ...
                print(
                    f"  Min/Max: {tf.reduce_min(quantity_prediction).numpy()}/{tf.reduce_max(quantity_prediction).numpy()}",
                )
        except Exception:
            pass
...
class SimilarityLayer(layers.Layer):
    ...
    def get_config(self):
        ...
        config.update(
            {
                "user_embedding_layer": tf.keras.layers.serialize(
    ...
    @classmethod
    def from_config(cls, config):
        user_embedding_layer = tf.keras.layers.deserialize(
            config.pop("user_embedding_layer"),
        )
...
@register_keras_serializable()
class ExplainableUnifiedModelWithPreprocessing(
    UnifiedRecommendationModelWithPreprocessing,
):
    def __init__(
        self,
        num_users,
