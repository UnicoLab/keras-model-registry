...
class PreprocessingLayer(Layer):
    ...
    def call(self, inputs):
        ...
        latitude = inputs['latitude'] * tf.constant(2 * 3.14159 / 360.0)  # shape: (batch_size,)
        longitude = inputs['longitude'] * tf.constant(2 * 3.14159 / 360.0)  # shape: (batch_size,)
        sales = inputs['sale_quantity']  # shape: (batch_size,)
        
        # Create a mask for filtering without expanding dimensions
        mask = tf.cast(tf.greater(sales, self.min_sales), dtype=tf.float32)  # shape: (batch_size,)
...
class DistanceLayer(Layer):
    ...
    def call(self, inputs):
        ...
        a = tf.sin(delta_lat / 2)**2 + \
            tf.cos(lat1) * tf.cos(tf.expand_dims(lat2, 1)) * \
...
class ClusteringLayer(Layer):
    ...
    def call(self, inputs, training=None):
        ...
        cluster_logits = tf.matmul(features, self.cluster_weights[:3, :])  # [batch_size, n_clusters]
...
class RankingLayer(Layer):
    ...
    def call(self, inputs, training=None):
        ...
        scaled_similarity = similarity / tf.sqrt(tf.cast(self.embedding_dim, tf.float32))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if 'indices' in outputs:
                ...
                entropy = -tf.reduce_mean(tf.reduce_sum(cluster_probs * tf.math.log(safe_probs), axis=1))
                
                # 2. Encourage confident assignments (minimize entropy per sample)
                confidence = -tf.reduce_mean(tf.reduce_sum(safe_probs * tf.math.log(safe_probs), axis=1))
            ...
            if 'scores' in outputs:
                ...
                # Adjusted weights for better balance
                loss_dict['scores'] = 0.5 * range_loss - 0.3 * mean_variance + 0.005 * sparsity
            
            # Compute total loss with L2 regularization
            l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in self.trainable_variables]) * 0.001
