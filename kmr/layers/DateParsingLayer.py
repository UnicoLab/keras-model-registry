...
class DateParsingLayer(BaseLayer):
    ...
    def _validate_date_format(self) -> None:
        ...
        if self.date_format not in supported_formats:
            raise ValueError(
                f"Unsupported date format: {self.date_format}. "
    ...
    def _parse_date(self, date_str):
        ...
        h = (day + ((13 * (adj_month + 1)) // 5) + adj_year + (adj_year // 4) - (adj_year // 100) + (adj_year // 400)) % 7
    ...
    def call(self, inputs):
        ...
        if hasattr(inputs, 'numpy'):
            ...
            date_strings = [s.decode('utf-8') if isinstance(s, bytes) else s for s in flat_inputs.numpy()]
