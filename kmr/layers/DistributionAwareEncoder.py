...
class DistributionAwareEncoder(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self._valid_distributions = [
            "normal", "exponential", "lognormal", "uniform", "beta",
            "bimodal", "heavy_tailed", "mixed", "bounded", "unknown"
        ]
    ...
    def _validate_params(self) -> None:
        ...
        if self._embedding_dim is not None and self._embedding_dim <= 0:
            raise ValueError(f"embedding_dim must be positive, got {self._embedding_dim}")
        ...
        if self._distribution_type not in self._valid_distributions:
            raise ValueError(
                f"distribution_type must be one of {self._valid_distributions}, "
                f"got {self._distribution_type}"
            )
    ...
    def build(self, input_shape: tuple[int, ...]) -> None:
        ...
        if self.add_distribution_embedding:
            ...
            self.distribution_embedding = self.add_weight(
                ...
                shape=(num_distributions, 8),  # 8-dimensional embedding for each distribution
    ...
    def _detect_distribution(self, x: KerasTensor) -> int:
        ...
        elif avg_skewness > 1.5 and ops.logical_not(has_negative) and ops.logical_not(is_bounded_01):
        ...
        elif avg_skewness > 2.0 and ops.logical_not(has_negative) and ops.any(x > 20 * ops.mean(mean)):
    ...
    def _calculate_skewness(self, x: KerasTensor, mean: KerasTensor = None, std: KerasTensor = None) -> KerasTensor:
    ...
    def _calculate_kurtosis(self, x: KerasTensor, mean: KerasTensor = None, std: KerasTensor = None) -> KerasTensor:
    ...
    def _check_bimodality(self, x: KerasTensor, mean: KerasTensor = None, std: KerasTensor = None, 
                         precalculated_kurtosis: KerasTensor = None) -> KerasTensor:
    ...
    def call(self, inputs: KerasTensor, training: bool | None = None) -> KerasTensor:
        ...
        if self.auto_detect:
            if training or not hasattr(self, "_is_initialized") or not self._is_initialized:
                ...
                distribution_idx = int(ops.convert_to_numpy(self.detected_distribution)[0])
                self.distribution_type = self._valid_distributions[distribution_idx]
        ...
        if self.projection is not None:
            encoded = self.projection(transformed)
        else:
            encoded = transformed
        ...
        if self.add_distribution_embedding and hasattr(self, "distribution_embedding"):
            ...
            if self.auto_detect:
                ...
                distribution_idx = self._valid_distributions.index(self.distribution_type)
            
            # Get the embedding for this distribution
            dist_embedding = self.distribution_embedding[distribution_idx]
            
            # Reshape for broadcasting
            dist_embedding = ops.reshape(dist_embedding, (1,) * (len(encoded.shape) - 1) + (8,))
