...
class GraphFeatureAggregation(BaseLayer):
    ...
    def _validate_params(self) -> None:
        ...
        if self.leaky_relu_alpha <= 0:
            raise ValueError(f"leaky_relu_alpha must be positive, got {self.leaky_relu_alpha}")
    ...
    def call(self, inputs: KerasTensor, training: bool | None = None) -> KerasTensor:
        ...
        h_i_tiled = ops.repeat(h_i, self.num_features, axis=2)  # (batch, num_features, num_features, embed_dim)
        h_j_tiled = ops.repeat(h_j, self.num_features, axis=1)  # (batch, num_features, num_features, embed_dim)
        
        # Now we can safely concatenate along the last dimension
        h_pairs = ops.concatenate([h_i_tiled, h_j_tiled], axis=-1)  # (batch, num_features, num_features, 2*embed_dim)
        
        # Reshape for matrix multiplication with attention vector
        h_pairs_flat = ops.reshape(h_pairs, (-1, 2 * self.embed_dim))  # (batch*num_features*num_features, 2*embed_dim)
        
        # Apply attention vector to get raw scores
        raw_scores_flat = ops.matmul(h_pairs_flat, self.attention_a)  # (batch*num_features*num_features, 1)
        ...
        raw_scores = ops.reshape(raw_scores_flat, (batch_size, self.num_features, self.num_features))
