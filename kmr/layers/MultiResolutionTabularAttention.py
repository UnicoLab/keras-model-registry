...
class MultiResolutionTabularAttention(BaseLayer):
    ...
    def build(self, input_shape: List[tuple[int, ...]]) -> None:
        ...
        if not isinstance(input_shape, list) or len(input_shape) != 2:
            raise ValueError(
                f"Input must be a list of two tensors (numerical and categorical features), "
                f"got {input_shape}"
            )
    ...
    def call(
        ...
        num_attention = self.num_attention(
            num_projected, num_projected, num_projected, training=training
        )
        ...
        cat_attention = self.cat_attention(
            cat_projected, cat_projected, cat_projected, training=training
        )
