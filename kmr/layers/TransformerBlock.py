...
class TransformerBlock(BaseLayer):
    ...
    def _validate_params(self) -> None:
        ...
        if not isinstance(self._dim_model, int) or self._dim_model <= 0:
            raise ValueError(f"dim_model must be a positive integer, got {self._dim_model}")
        ...
        if not isinstance(self._ff_units, int) or self._ff_units <= 0:
            raise ValueError(f"ff_units must be a positive integer, got {self._ff_units}")
    ...
    def build(self, input_shape: tuple[int, ...]) -> None:
        ...
        logger.debug(f"TransformerBlock built with dim_model={self.dim_model}, num_heads={self.num_heads}")
