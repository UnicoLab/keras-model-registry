...
class VariableSelection(BaseLayer):
    ...
    def _validate_params(self) -> None:
        ...
        if not isinstance(self._nr_features, int) or self._nr_features <= 0:
            raise ValueError(f"nr_features must be a positive integer, got {self._nr_features}")
    ...
    def build(self, input_shape: Union[tuple[int, ...], List[tuple[int, ...]]]) -> None:
        ...
        if self.use_context:
            if not isinstance(input_shape, list) or len(input_shape) != 2:
                raise ValueError(
                    f"When use_context is True, input_shape must be a list of two shapes, "
                    f"got {input_shape}"
                )
            ...
            if len(context_shape) != 2:
                raise ValueError(
                    f"Context tensor must be 2-dimensional (batch_size, context_dim), "
                    f"got shape {context_shape}"
                )
            
            context_dim = context_shape[-1]
        else:
    ...
    def call(
        self, 
        inputs: Union[KerasTensor, List[KerasTensor]], 
        training: bool = False
        ...
        if self.use_context:
            if not isinstance(inputs, list) or len(inputs) != 2:
                raise ValueError(
                    "When use_context is True, inputs must be a list of two tensors "
            ...
            if len(features.shape) == 2:
                features = ops.expand_dims(features, axis=-1)  # [batch_size, nr_features, 1]
        else:
            # When use_context is False, we expect a single tensor, not a list
            if isinstance(inputs, list):
                raise ValueError(
                    "When use_context is False, inputs must be a single tensor, "
