...
class TerminatorModel(BaseModel):
    ...
    def call(self, inputs: List[KerasTensor] | KerasTensor, training: bool = False) -> KerasTensor:
        ...
        if isinstance(inputs, list):
            ...
            raise ValueError("TerminatorModel expects a list of inputs [input_tensor, context_tensor]")
        
        # Apply preprocessing if available
        if self.preprocessing_model is not None:
            x = self.preprocessing_model(x, training=training)
        
        logger.debug(f"TerminatorModel input shape: {x.shape}, context shape: {context.shape}")
        ...
        context_features = self.context_dense(context)
