...
class KMRPlotter:
    ...
    def plot_training_history(
        ...
        fig = make_subplots(
            rows=rows, cols=cols,
            subplot_titles=[f'Training and Validation {metric.title()}' for metric in metrics]
        )
    ...
    def _create_anomaly_detection_plot(
        ...
        fig.add_trace(go.Histogram(x=normal_scores, name='Normal', opacity=0.7, nbinsx=30), row=1, col=1)
        fig.add_trace(go.Histogram(x=anomaly_scores, name='Anomaly', opacity=0.7, nbinsx=30), row=1, col=1)
        ...
        fig.add_trace(go.Heatmap(z=cm_matrix, 
    ...
    def _create_classification_plot(
        ...
        if y_scores is not None:
            fig.add_trace(go.Histogram(x=y_scores, name='Scores', nbinsx=30), row=2, col=1)
    ...
    def _create_regression_plot(
        ...
        fig.add_trace(go.Histogram(x=residuals, name='Residuals', nbinsx=30), row=2, col=2)
