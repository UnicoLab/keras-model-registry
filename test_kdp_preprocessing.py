...
def test_data_generation():
    ...
    # Verify balance
    balance_ratio_train = train_balance[1] / train_balance[0] if train_balance[0] > 0 else 0
...
def test_simple_preprocessing_model(X_train, X_test, y_train, y_test):
    ...
    preprocessing_model = Model(inputs=input_layer, outputs=output_layer, name='simple_preprocessor')
...
def test_kdp_preprocessing_model(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        if test_precision == 0.0 and test_recall == 0.0:
            ...
            print(f"   KDP output range: [{kdp_output.min():.4f}, {kdp_output.max():.4f}]")
            print(f"   KDP output std: {kdp_output.std():.4f}")
            
            if kdp_output.std() < 0.01:
                print("   ⚠️ KDP output has very low variance - information bottleneck!")
            if kdp_output.shape[1] < X_train.shape[1]:
                print("   ⚠️ KDP is reducing dimensionality - may be losing information!")
...
def test_improved_kdp_preprocessing(X_train, X_test, y_train, y_test):
    ...
    try:
        ...
        features_specs = {}
        for i in range(X_train.shape[1]):
            features_specs[f'num_feature_{i}'] = FeatureType.FLOAT_RESCALED  # Use rescaling instead of normalization
        ...
        print(f"✅ Improved KDP preprocessing test passed! Output shape: {improved_sample.shape}")
        ...
        # Evaluate model
        test_loss, test_accuracy, test_precision, test_recall = model_with_improved_kdp.evaluate(
            test_data, y_test, verbose=0
        )
...
def run_all_tests():
    ...
    # Test 2: Basic TerminatorModel
    basic_model, basic_ok, basic_f1 = test_basic_terminator_model(X_train, X_test, y_train, y_test)
    
    # Test 3: Simple Preprocessing Model
    simple_model, simple_ok, simple_f1 = test_simple_preprocessing_model(X_train, X_test, y_train, y_test)
    
    # Test 4: KDP Preprocessing Model
    kdp_model, kdp_ok, kdp_f1 = test_kdp_preprocessing_model(X_train, X_test, y_train, y_test)
    
    # Test 5: Improved KDP Preprocessing Model
    improved_kdp_model, improved_kdp_ok, improved_kdp_f1 = test_improved_kdp_preprocessing(X_train, X_test, y_train, y_test)
    
    # Summary
    print("\n📊 Test Results Summary")
    print("=" * 60)
    print(f"Data Generation: {'✅ PASS' if data_ok else '❌ FAIL'}")
    print(f"Basic TerminatorModel: {'✅ PASS' if basic_ok else '❌ FAIL'} (F1: {basic_f1:.4f})")
    print(f"Simple Preprocessing: {'✅ PASS' if simple_ok else '❌ FAIL'} (F1: {simple_f1:.4f})")
    print(f"KDP Preprocessing: {'✅ PASS' if kdp_ok else '❌ FAIL'} (F1: {kdp_f1:.4f})")
    print(f"Improved KDP: {'✅ PASS' if improved_kdp_ok else '❌ FAIL'} (F1: {improved_kdp_f1:.4f})")
