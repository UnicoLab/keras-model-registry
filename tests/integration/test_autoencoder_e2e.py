...
class TestAutoencoderE2E:
    ...
    def test_end_to_end_without_preprocessing(
        ...
        # Train the model
        history = model.fit(
            X_train, X_train,  # Autoencoder target is same as input
            epochs=5,
            batch_size=32,
        ...
        model_path = temp_dir / "saved_autoencoder_no_preprocessing.keras"
    ...
    def test_end_to_end_with_kdp_preprocessing(
        ...
        # Test model saving and loading
        model_path = temp_dir / "saved_autoencoder_with_kdp.keras"
        model.save(model_path)
