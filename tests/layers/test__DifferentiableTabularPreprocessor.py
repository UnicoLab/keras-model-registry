...
class TestDifferentiableTabularPreprocessor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_input = tf.where(mask, tf.constant(float('nan'), dtype=tf.float32), self.test_input)
    ...
    def test_output_shape(self) -> None:
        ...
        for shape in test_shapes:
            ...
            test_input = tf.where(mask, tf.constant(float('nan'), dtype=tf.float32), test_input)
    ...
    def test_normalization(self) -> None:
        ...
        expected_output = tf.constant([
            [3.0, 5.0, 7.0, 9.0],  # 2*[1,2,3,4] + 1
            [11.0, 13.0, 15.0, 17.0]  # 2*[5,6,7,8] + 1
        ], dtype=tf.float32)
    ...
    def test_serialization(self) -> None:
        ...
        original_layer = DifferentiableTabularPreprocessor(num_features=self.num_features)
    ...
    def test_learnable_weights(self) -> None:
        ...
        # Create a simple model with the preprocessing layer
        inputs = layers.Input(shape=(self.num_features,))
        preproc_layer = DifferentiableTabularPreprocessor(num_features=self.num_features)
