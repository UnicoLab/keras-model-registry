...
class TestDistributionAwareEncoder(unittest.TestCase):
    ...
    def test_training_mode(self) -> None:
        ...
        self.assertFalse(
            np.allclose(inference_output, inference_after_output, atol=1e-5),
        )
    ...
    def test_statistics_calculation(self) -> None:
        ...
        self.assertTrue(
            np.mean(np.abs(skewness_np)) < 1.0,
        )  # Normal data should have relatively low skewness

        # Test skewness calculation on exponential data
        skewness = encoder._calculate_skewness(self.exp_data)
        skewness_np = ops.convert_to_numpy(skewness)
        self.assertTrue(
            np.mean(skewness_np) > 1.0,
        )  # Exponential data should have positive skewness
        ...
        kurtosis_np = ops.convert_to_numpy(kurtosis)
        self.assertTrue(
            np.any(kurtosis_np > 3.0),
        )  # Heavy-tailed data should have high kurtosis
