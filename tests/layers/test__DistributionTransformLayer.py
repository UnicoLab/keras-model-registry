...
class TestDistributionTransformLayer(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.pos_skewed_data = tf.convert_to_tensor(
            np.random.exponential(scale=1.0, size=(self.batch_size, self.feature_dim)).astype(np.float32)
        )
        
        # Normal data
        self.normal_data = tf.convert_to_tensor(
            np.random.normal(size=(self.batch_size, self.feature_dim)).astype(np.float32)
        )
    ...
    def test_output_shape(self) -> None:
        ...
        transforms = [
            ...
            "arcsinh", "cube-root", "logit", "quantile", "robust-scale", "min-max"
        ]
    ...
    def test_yeo_johnson_transform(self) -> None:
        ...
        layer = DistributionTransformLayer(transform_type="yeo-johnson", lambda_param=0.0)
        output = layer(self.pos_skewed_data)
        ...
        layer = DistributionTransformLayer(transform_type="yeo-johnson", lambda_param=2.0)
        ...
        layer = DistributionTransformLayer(transform_type="yeo-johnson", lambda_param=1.0)
    ...
    def test_training_mode(self) -> None:
        ...
        self.assertTrue(tf.reduce_all(tf.math.abs(output_train - output_infer) < 1e-5),
    ...
    def test_integration(self) -> None:
        ...
        x_data = tf.convert_to_tensor(
            np.random.exponential(scale=1.0, size=(100, self.feature_dim)).astype(np.float32)
        )
    ...
    def test_distribution_normalization(self) -> None:
        ...
        box_cox_layer = DistributionTransformLayer(transform_type="box-cox", lambda_param=0.5)
    ...
    def test_arcsinh_transform(self) -> None:
        ...
        expected = tf.math.log(self.mixed_data + tf.sqrt(tf.square(self.mixed_data) + 1.0))
    ...
    def test_logit_transform(self) -> None:
        ...
        uniform_data = tf.convert_to_tensor(
            np.random.uniform(0.1, 0.9, size=(self.batch_size, self.feature_dim)).astype(np.float32)
        )
        ...
        outside_range = tf.convert_to_tensor(
            np.random.uniform(-1.0, 2.0, size=(self.batch_size, self.feature_dim)).astype(np.float32)
        )
    ...
    def test_min_max_transform(self) -> None:
        ...
        self.assertTrue(tf.reduce_mean(tf.cast(output_no_clip >= 0.0, tf.float32)) > 0.9)
        self.assertTrue(tf.reduce_mean(tf.cast(output_no_clip <= 1.0, tf.float32)) > 0.9)
    ...
    def test_auto_transform_positive_skewed(self) -> None:
        ...
        selected_lambda = float(layer._selected_lambda.numpy()[0])
    ...
    def test_auto_transform_serialization(self) -> None:
        ...
        # Apply transformation in training mode
        original_output = original_layer(data, training=True)
        restored_output = restored_layer(data, training=True)
        ...
        self.assertTrue(tf.reduce_all(tf.abs(original_output_infer - restored_output_infer) < 1e-5))
    ...
    def test_auto_transform_distribution_recognition(self) -> None:
        """Test that auto mode correctly recognizes different distribution types and selects appropriate transformations."""
        ...
        def check_distribution_transform(data, name, expected_transforms, candidates=None):
            ...
            original_kurtosis = self._calculate_kurtosis(data_tensor.numpy())
            print(f"\n{name} distribution statistics:")
            print(f"  - Skewness: {original_skew:.4f}")
            print(f"  - Kurtosis: {original_kurtosis:.4f}")
            print(f"  - Has negative values: {tf.reduce_any(data_tensor < 0).numpy()}")
            print(f"  - Has zeros: {tf.reduce_any(tf.equal(data_tensor, 0.0)).numpy()}")
            print(f"  - Is bounded (0,1): {self._is_bounded_01(data_tensor)}")
            ...
            self.assertIn(selected_transform, expected_transforms, 
            ...
            transformed_kurtosis = self._calculate_kurtosis(output.numpy())
            ...
            if name != "normal" and name != "t-distribution" and name != "beta" and name != "negative-skewed" and abs(original_skew) > 0.1:
                self.assertLess(abs(transformed_skew), abs(original_skew), 
                               f"For {name} distribution, transformation did not reduce skewness")
            
            # Log the selected transformation and skewness reduction
            print(f"{name} distribution: selected {selected_transform} (λ={selected_lambda}), "
                  f"skewness reduced from {original_skew:.4f} to {transformed_skew:.4f}")
            print(f"Kurtosis changed from {original_kurtosis:.4f} to {transformed_kurtosis:.4f}")
        ...
        check_distribution_transform(
            normal_data, 
            "normal", 
            ["none", "yeo-johnson", "arcsinh", "cube-root", "robust-scale", "quantile", "min-max"]
        )
        ...
        check_distribution_transform(
            bimodal_data, 
            "bimodal", 
            ["robust-scale", "quantile", "yeo-johnson", "arcsinh", "cube-root", "min-max"]
        )
