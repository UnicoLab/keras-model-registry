...
class TestGraphFeatureAggregation(unittest.TestCase):
    ...
    def test_attention_mechanism(self) -> None:
        ...
        pattern2 = tf.concat(
            [
                tf.ones((1, self.num_features // 2)),
                tf.zeros((1, self.num_features // 2)),
            ],
            axis=1,
        )
    ...
    def test_residual_connection(self) -> None:
        ...
        modified_layer(test_input)

        # Now modify the layer to have zero output projection
        if hasattr(modified_layer.out_proj, "kernel"):
            modified_layer.out_proj.kernel.assign(
                tf.zeros_like(modified_layer.out_proj.kernel),
            )
        if hasattr(modified_layer.out_proj, "bias"):
            modified_layer.out_proj.bias.assign(
                tf.zeros_like(modified_layer.out_proj.bias),
            )
        ...
        self.assertTrue(
            np.allclose(zero_transform_output.numpy(), test_input.numpy(), atol=1e-5),
        )
