"""Unit tests for MultiScaleSeasonMixing layer.
...
class TestMultiScaleSeasonMixing(unittest.TestCase):
    """Test cases for MultiScaleSeasonMixing layer."""
    ...
    def setUp(self) -> None:
        ...
        self.layer = MultiScaleSeasonMixing(
    ...
    def test_output_type(self) -> None:
        ...
        seasonal_inputs = [tf.random.normal((self.batch_size, self.seq_len, 8))]
        outputs = self.layer(seasonal_inputs)
        self.assertIsInstance(outputs, list)

    def test_output_shapes(self) -> None:
        """Test output shapes for different downsampling layers."""
        seasonal_inputs = [tf.random.normal((self.batch_size, self.seq_len, 8))]
        outputs = self.layer(seasonal_inputs)
    ...
    def test_different_batch_sizes(self) -> None:
        ...
        for batch_size in [1, 8, 32]:
            seasonal_inputs = [tf.random.normal((batch_size, self.seq_len, 8))]
            outputs = self.layer(seasonal_inputs)
            self.assertIsInstance(outputs, list)
            self.assertGreater(len(outputs), 0)
    ...
    def test_serialization(self) -> None:
        ...
        new_layer = MultiScaleSeasonMixing.from_config(config)
    ...
    def test_deterministic_output(self) -> None:
        ...
        outputs1 = self.layer(seasonal_inputs, training=False)
        outputs2 = self.layer(seasonal_inputs, training=False)
