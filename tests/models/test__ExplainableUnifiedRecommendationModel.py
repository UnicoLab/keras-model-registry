"""Tests for ExplainableUnifiedRecommendationModel."""
...
class TestExplainableUnifiedRecommendationModel(unittest.TestCase):
    """Test suite for ExplainableUnifiedRecommendationModel."""
    ...
    def test_forward_pass_output_shapes(self) -> None:
        """Test that forward pass produces 5 outputs."""
        ...
        # Should return 5 outputs with correct batch dimensions
        self.assertEqual(len(results), 5)
        rec_indices, rec_scores, cf_sim, cb_sim, weights = results
        self.assertEqual(rec_indices.shape[0], self.batch_size)
        self.assertEqual(rec_scores.shape[0], self.batch_size)
        self.assertEqual(cf_sim.shape[0], self.batch_size)
        self.assertEqual(cb_sim.shape[0], self.batch_size)
    ...
    def test_output_indices_valid(self) -> None:
        ...
        rec_indices, _, _, _, _ = self.model(
            [user_ids, user_features, item_ids, item_features],
            training=False,
        )
    ...
    def test_weights_sum_to_one(self) -> None:
        """Test that weights sum to 1 (softmax)."""
        user_ids = tf.constant(np.random.randint(0, self.num_users, (self.batch_size,)))
        user_features = tf.constant(
            np.random.randn(self.batch_size, self.user_feature_dim).astype(np.float32),
        )
        ...
        _, _, _, _, weights = self.model(
            [user_ids, user_features, item_ids, item_features],
            training=False,
        )

        weights_sum = tf.reduce_sum(weights).numpy()
    ...
    def test_different_batch_sizes(self) -> None:
        ...
        for batch_size in [1, 8, 16]:
            ...
            rec_indices, rec_scores, cf_sim, cb_sim, weights = self.model(
                [user_ids, user_features, item_ids, item_features],
                training=False,
            )
            ...
            self.assertEqual(cf_sim.shape[0], batch_size)
            self.assertEqual(cb_sim.shape[0], batch_size)
    ...
    def test_deserialization(self) -> None:
        ...
        config = self.model.get_config()
        new_model = ExplainableUnifiedRecommendationModel.from_config(config)
    ...
    def test_inference_consistency(self) -> None:
        ...
        idx1, scores1, cf1, cb1, w1 = self.model(
            [user_ids, user_features, item_ids, item_features],
            training=False,
        )
        idx2, scores2, cf2, cb2, w2 = self.model(
            [user_ids, user_features, item_ids, item_features],
            training=False,
        )

        np.testing.assert_array_equal(idx1.numpy(), idx2.numpy())
        np.testing.assert_array_almost_equal(cf1.numpy(), cf2.numpy())
