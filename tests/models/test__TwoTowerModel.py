...
class TestTwoTowerModel(unittest.TestCase):
    ...
    def test_initialization_default(self) -> None:
        ...
        self.assertEqual(model.output_dim, 32)
    ...
    def test_invalid_item_feature_dim(self) -> None:
        """Test that invalid item_feature_dim raises error."""
        with self.assertRaises(ValueError):
            TwoTowerModel(
                user_feature_dim=32,
                item_feature_dim=-1,
    ...
    def test_output_scores_in_valid_range(self) -> None:
        """Test that output scores are in valid range."""
        user_features = tf.constant(
            np.random.randn(self.batch_size, self.user_feature_dim).astype(np.float32),
        )
        item_features = tf.constant(
        ...
        self.assertTrue(np.all(np.isfinite(rec_scores.numpy())))
    ...
    def test_different_feature_dimensions(self) -> None:
        """Test with different feature dimensions."""
        for feat_dim in [16, 32, 64]:
            model = TwoTowerModel(
                user_feature_dim=feat_dim,
                item_feature_dim=feat_dim,
                num_items=self.num_items,
            )
            user_features = tf.constant(
                np.random.randn(self.batch_size, feat_dim).astype(np.float32),
            )
            item_features = tf.constant(
                np.random.randn(self.batch_size, self.num_items, feat_dim).astype(
                    np.float32,
                ),
            )

            rec_indices, _ = model([user_features, item_features], training=False)
    ...
    def test_deserialization(self) -> None:
        ...
        config = self.model.get_config()
        new_model = TwoTowerModel.from_config(config)
        ...
        self.assertEqual(new_model.output_dim, self.model.output_dim)
