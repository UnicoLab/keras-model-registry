...
class TestDataAnalyzerCLI(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        sample_data = pd.DataFrame({"num1": [1.0, 2.0, 3.0], "cat1": ["A", "B", "C"]})
    ...
    def test_main_stdout(self, mock_stdout, mock_analyzer_class) -> None:
        ...
        mock_analyzer.analyze_and_recommend.assert_called_once_with(
            self.csv_path,
            "*.csv",
        )
    ...
    def test_main_file_output(self, mock_analyzer_class) -> None:
        ...
        with mock.patch(
            "sys.argv",
            ["data_analyzer_cli.py", self.csv_path, "--output", output_path],
        ):
            main()
